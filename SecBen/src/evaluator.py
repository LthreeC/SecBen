...
from src.service import get_model_service
...
def simple_evaluate(
    ...
    if isinstance(model, str):
        if model_args is None:
            model_args = ""
        if any(sub in model for sub in ["/", "\\"]):
            import socket
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            if s.connect_ex(("127.0.0.1", 50000)) == 0:
                lm = get_model_service()
            else:
...
t in range(turn)], turn)
    ...
    if write_out:
        ...
        output_name = f"{output_base_path}~{task_name}~{timestamp}~write_out_info.json"
